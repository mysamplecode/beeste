	...
	<div class="javo_mhome_map_lists">
		...
		<div class="main-map-search-wrap">
			<div class="row">
				...
				<div class="col-md-3 category-list text-left">
					...
					<div class="javo-filter-column" style="display:none;">
						...
						<div class="javo-filter-column" style="display: none;">
							<div class="sel-box">
								<div class="sel-container">
									...
									<input type="hidden" id="javo-map-box-ppp" value="20">
								</div><!-- /.sel-container -->
				...
				<div class="col-md-6 map-inner-search" style="padding-left: 1px;">
					
					<div class="input-group input-group-sm" data-javo-hmap-keyword-search style="width: 186px; display:none;">
					...
					<?php echo javo_single_search_form::javo_search_form_callback ( array("place_holder" => "Where would you like a massage?",  "input_field" => "location",  "action" => 695, "single_search_button_color" => "#717171", 'class' => 'bestee-inner-search') );?>
				</div>
				<div class="col-md-3 map-gride-view">
					<div class="row">
						<div class="col-md-8">
...
<!--# Templates -->
				...
				<div class="caption">
					...
					<div class="row">
						...
						<div class="col-xs-8 col-sm-8 col-md-8">
							<div class="row">
								<div class="col-md-12"><span class="map-box-massage-bold">
                                                                                    </span>{post_price} (60mins)</div><!-- col md 8 -->
							</div><!-- Row -->
						</div>
	...
	<script type="text/template" id="javo-map-box-panel-content-featured">
		<div class="item col-md-6 col-xs-12 featured-list">
			<div class="thumbnail item-list-box-map">
				<div class="thumb-wrap">
					{thumbnail_large}
					<div class="javo-left-overlay">
					...
					<span class="feature-span">Featured</span>
					<div class="rate-icons">
						<?php if( $javo_custom_item_tab->get('ratings', '') == ''): ?>
							<div class="col-md-2">
								<div class="col-md-12 javo-rating-registed-score" data-score="{rating}"></div>
	...
	<script type="text/template" id="javo-map-box-infobx-content">
		...
		<div class="javo_somw_info panel" style="min-height:220px;">
			...
			<div class="pics">
				...
				<div class="javo-left-overlay no-location">
...
<fieldset>
	...
	if( 'use' !== $javo_tso->get( 'cross_doamin', '' ) ) {
        ...
        $json_content_url = "{$upload_folder['baseurl']}/javo_ajax_content_url.php";
        ?>

	<!--# Variables -->
	<input type="hidden" javo-ajax-url value="<?php echo admin_url( 'admin-ajax.php' );?>">
        <input type="hidden" javo-ajax-content-url value="<?php echo $json_content_url;?>">
...
//				var json_ajax_url		= $( "[javo-map-all-items]").val();
			...
			{
				//console.log("4. --------"+parseInt( $( "#javo-map-box-ppp" ).val() )+"-------- 4.");
				var obj			= window.javo_map_box_func;
					...
					, function( response )
					{
						...
						if( response.length > 0 )
						{
							$.each( response, function( index, data ){
								var str = "";
								if(data.is_featured === 'featured')
								{
								    str = $('#javo-map-box-panel-content-featured').html();
								}
								else
								{
								    str = $('#javo-map-box-panel-content').html();
								}
								var price = data.post_price;
                                                                price = price.replace(/\$/g, '');//("$","");
                                                                if(price=="" || typeof price=="undefined" || price.toLowerCase()=="not available" || price.toLowerCase()=="n/a")
                                                                    price = "-";
                                                                else{
                                                                    price = "$"+price;
                                                                }
								str = str.replace(/{post_id}/g			, data.post_id );
								str = str.replace(/{post_price}/g		, price );
						...
						obj.el.gmap3({
							getlatlng:{
								...
								, callback: function( response )
								{
									...
									var offset = 0.0005;
									if( metry.viewport ) {
										var xx = metry.viewport.getSouthWest().lat() * ((metry.viewport.getSouthWest().lat() < 0)?(1+offset):(1-offset));
										var xy = metry.viewport.getNorthEast().lat() * ((metry.viewport.getNorthEast().lat() < 0)?(1-offset):(1+offset));
										var yx = metry.viewport.getSouthWest().lng() * ((metry.viewport.getSouthWest().lng() < 0)?(1+offset):(1-offset));
										var yy = metry.viewport.getNorthEast().lng() * ((metry.viewport.getNorthEast().lng() < 0)?(1-offset):(1+offset));
										map.fitBounds( metry.viewport );
										sanitize_result = obj.latlng_calc( metry.location.lat(), metry.location.lng(), xx, xy, yx, yy, data );
									}
			...
			, latlng_calc: function( lat, lng, s, e, n, w, item ){
				
				
				function sorter(a,b) 
				{
				    var temp = parseFloat(a.diff) - parseFloat(b.diff);
				    return temp;
				}
				...
				var temp = [];
                                ...
                                // first append featured items
                                $.each( item, function( i, k ){
                                        dist = apply_distance_filter({lat1: lat,lon1: lng,lat2: k.lat,lon2: k.lng});
					if(
                                                ( s <= parseFloat( k.lat) && e >= parseFloat(k.lat ) ) &&
						( n <= parseFloat( k.lng) && w >= parseFloat(k.lng ) ) &&
                                                ( k.is_featured==="featured" && typeof (k.is_featured)!=="undefined" && k.is_featured!=="use" ) &&
                                                ( dist.km <= distance_limit )
                                          ){
						var a_lat = parseFloat(k.lat) - parseFloat(lat, 10);
						var b_lat = parseFloat(k.lng) - parseFloat(lng, 10);
						var res = Math.abs(a_lat + b_lat);
						temp_featured.push( {code:item[i], diff:res } );
					}
				} );
				$.each( item, function( i, k ){
                                        dist = apply_distance_filter({lat1: lat,lon1: lng,lat2: k.lat,lon2: k.lng});
					if(
						( s <= parseFloat( k.lat) && e >= parseFloat(k.lat ) ) &&
						( n <= parseFloat( k.lng) && w >= parseFloat(k.lng ) ) &&
                                                ( k.is_featured!=="featured" || typeof k.is_featured=="undefined" ) &&
                                                ( dist.km <= distance_limit )
					){
						var a_lat = parseFloat(k.lat) - parseFloat(lat, 10);
						var b_lat = parseFloat(k.lng) - parseFloat(lng, 10);
						var res = Math.abs(a_lat + b_lat);
						temp.push( {code:item[i], diff:res } );
					}
				} );
                                temp_featured.sort(sorter);
				temp.sort(sorter);
                                $.each( temp_featured, function( i, k ){
					result.push(temp_featured[i].code);
				} );
				$.each( temp, function( i, k ){
					result.push(temp[i].code);
				} );
				return result;
			}
		...
		window.javo_map_box_func.init();
                ...
                function apply_distance_filter(obj) {
                    var R = 6371; // km
